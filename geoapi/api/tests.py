...
class IpGeolocationTests(APITestCase):
    ...
    def test_fetching_list_of_geodata(self):
        ...
        self.assertContains(response, 'Split-Dalmatia',
    ...
    def test_posting_new_geodata_ipv6(self):
        ...
        self.assertContains(
            response, ipv6, status_code=status.HTTP_201_CREATED)
    ...
    def test_posting_new_geodata_ipv4(self):
        ...
        self.assertContains(
            response, ipv4, status_code=status.HTTP_201_CREATED)

        # verify that the newly added data is accessible
        response = self.client.get(
            reverse('geodata-detail', kwargs={'ip': ipv4}), format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
